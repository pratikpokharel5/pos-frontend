...
import type { Product, ProductImportResult } from "../types/productTypes";
...
async function importProducts() {
  ...
  try {
    const response = await productsApi.import(importFile.value);

    importResult.value = response.data;
    await loadProducts();
  } catch (err) {
    importError.value = err instanceof Error ? err.message : "Unable to import products.";
  } finally {
...
<template>
  <Dialog
    ...
    subtitle="Upload an Excel file with product data."
  ...
  >
    <div class="grid gap-4">
      <FileInput
        label="Excel File"
        accept=".xlsx,.xls"
        description="Expected columns: name, price, sku, category, description, status."
        :disabled="importing"
        v-model="importFile"
      />
  ...
  <section>
    <PageHeader
      eyebrow="Catalog"
      title="Products"
      description="Add, edit, and manage your products in the catalog."
    >
      <template #actions v-if="auth.isAdmin">
        <div class="flex flex-wrap gap-2">
          ...
          <Button to="/products/add" variant="primary">
            <Plus :size="18" />
            Add Product
          </Button>
        </div>
    ...
    <section class="rounded-app border-line bg-surface shadow-app border">
      <div class="border-line flex justify-between gap-3 border-b p-4 max-md:grid">
        <div>
          <h3 class="m-0 text-base font-black">Search Catalog</h3>
          <p class="text-muted mt-1 mb-0 text-sm">Find products by name, SKU, or status.</p>
        </div>
      </div>
      ...
      >
        <InputField label="Search" placeholder="Laptop, coffee, SKU..." v-model="search" />
    ...
    <section class="rounded-app border-line bg-surface shadow-app mt-4 border p-4">
      <DataTable
        ...
        empty-description="Add products to make sale entry faster."
      >
        ...
        <template #cell-actions="{ row }">
          <div class="flex gap-2" v-if="auth.isAdmin">
            <Button :to="`/products/${row.id}/edit`" size="icon" title="Edit product">
